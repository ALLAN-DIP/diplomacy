...
export const PanelChooseMap = (props) => {
    const mapImg = require(`../../../diplomacy/maps/svg/${props.params.map.svgName()}.svg`);
    const mapEntries = [];
    let count = 0;
            ...
            mapEntries.push(
                <div key={count} className="mb-1 d-flex flex-row">
                    <button
                        ...
                        onMouseOver={() => props.onUpdateParams({ map: mapInfo })}
                ...
                <div key={count}>
                    <div className="mb-1 d-flex flex-row">
                        <button
                            type="button"
                            className="btn btn-secondary btn-sm flex-grow-1 mr-1"
                            onMouseOver={() => props.onUpdateParams({ map: defaultVariant })}
                            onClick={() => props.forward()}
                        >
                                    ...
                                    views.push(
                                        <div key={variantInfo.name} className="mb-1">
                                            <button
                                                ...
                                                onMouseOver={() => props.onUpdateParams({ map: variantInfo })}
                                                onClick={() => props.forward()}
                                            >
                                                {variantInfo.title}
                                            </button>
    ...
    return (
        <FancyBox title={"Choose a map"} onClose={props.cancel}>
            <div className="row panel-choose-map">
                <div className="col-md">
                    <div className="map-list p-1 ml-0 ml-sm-1">{mapEntries}</div>
                </div>
                <div className="col-md">
                    <img className="img-fluid" src={mapImg} alt={props.params.map.title} />
                </div>
            </div>
        </FancyBox>
    );
