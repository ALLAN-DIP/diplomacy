...
export class SvgStandard extends React.Component {
  ...
  onPrediction(orderBuilding, province) {
    ...
    if (phaseType === "M") {
      ...
      for (const type of orderTypes) {
        ...
        const orderTypeLocations = UTILS.javascript.getTreeValue(
          possibleOrders,
          type,
        );
  ...
  renderOrderFromDist(order, powerName, game, opacity, key) {
    ...
    if (tokens[2] === "H") {
      ...
      const destLoc =
        tokens[tokens.length - (tokens[tokens.length - 1] === "VIA" ? 2 : 1)];
      renderComponents.renderedOrders.push(
        <Move
          key={`${key}:${order}`}
          opacity={opacity}
  ...
  render() {
    ...
    if (this.props.orderBuilding && this.props.orderBuilding.path.length) {
      ...
      if (neighbors) {
        for (let neighbor of neighbors) {
          ...
          if (!neighborID)
            throw new Error(
              `Unknown neighbor path (${neighborID}) for province (${neighbor}).`,
            );
    ...
    return (
      <svg
        className="SvgStandard noselect"
      ...
      >
        ...
        <text
          className={classes["CurrentNote2"]}
          id="CurrentNote2"
          x="35"
          y="85"
        >
